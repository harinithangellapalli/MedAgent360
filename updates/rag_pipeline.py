...
def run_full_pipeline(pdf_path: str, language: str = "English") -> dict:
    ...
    if not parsed["lab_values"]:
        raise ValueError("No lab values could be extracted from this PDF. Please check the file format.")

    # A3–A4: Classify
    classified = classify_lab_values(parsed["lab_values"])

    # A5: Summarise
    summary = generate_summary(classified, parsed["patient_info"], language)
